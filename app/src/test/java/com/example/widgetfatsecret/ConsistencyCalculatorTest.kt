...
class ConsistencyCalculatorTest {
    ...
    @Test
    fun emptyHistoryIsAllMissingWithZeroStreaks() {
        val s = ConsistencyCalculator.summarize(emptyList(), windowDays = 5, today = 104)
    ...
    fun futureDaysAreNeverMissingAndDoNotBreakTheCurrentStreak() {
        ...
        val s = ConsistencyCalculator.summarize(
            listOf(day(100), day(101)),
            windowStart = 99,
            windowEnd = 103,
            today = 101,
        )
        val byDate = s.days.associateBy { it.dateInt }
        assertEquals(ConsistencyDayState.MISSING, byDate.getValue(99).state)
    ...
    fun aGapInTheMiddleBreaksBothStreaksCorrectly() {
        ...
        val s = ConsistencyCalculator.summarize(history, windowDays = 5, today = 100)
    ...
    fun aMonthBoundaryDoesNotAffectStreakMath() {
        ...
        val s = ConsistencyCalculator.summarize(history, windowDays = 3, today = 32)
