...
enum class ConsistencyDayState { RECORDED, MISSING, FUTURE }
...
data class ConsistencySummary(
    ...
    val recordedPercent: Float get() = if (windowDays == 0) 0f else daysRecorded.toFloat() / windowDays
}
 ...
 * never [ConsistencyDayState.MISSING]: it must not count against a streak or
...
object ConsistencyCalculator {
    ...
    fun summarize(history: List<DayNutrition>, windowDays: Int, today: Long): ConsistencySummary =
        summarize(history, windowStart = today - windowDays + 1, windowEnd = today, today = today)

    fun summarize(history: List<DayNutrition>, windowStart: Long, windowEnd: Long, today: Long): ConsistencySummary {
        ...
        val days = (windowStart..windowEnd).map { d ->
            val state = when {
                ...
                else -> ConsistencyDayState.MISSING
            }
        ...
        for (day in days) {
            when (day.state) {
                ...
                ConsistencyDayState.MISSING -> running = 0
